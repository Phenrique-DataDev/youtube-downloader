...
| A-005 | Rodar do IP residencial evita a detecção de bot na maioria dos usos | **Validada em 2026-07-20** (yt-dlp `2026.07.04`, Windows, IP residencial): 5 sondas `-J` consecutivas + 1 download real, todas sem bloqueio nem rate limit. Cobre uso individual manual; uso intenso sustentado não foi testado (wiki documenta ~300 vídeos/h para sessão guest) | [x] |
| A-009 | Extração do YouTube segue funcionando **sem** runtime JavaScript | O yt-dlp `2026.07.04` avisa que isso está **deprecado**. Medido hoje: **zero perda** (29 formatos idênticos com e sem Node, H.264 1080p presente). Se o caminho fechar, o bootstrap ganha uma **4ª dependência** (Deno/Node), com impacto em SC-4 e no tamanho do cache | [x] parcial — vale hoje, com prazo indefinido |
